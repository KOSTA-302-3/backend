...
public interface FollowRepository extends JpaRepository<Follows, Long> {
     ...
     */
    List<Follows> findByFollower_UserId(Long id);

    /**
     * 팔로잉 목록 보기 (페이징)
     */
    Page<Follows> findByFollower_UserId(Long id, Pageable pageable);

    /**
     * 팔로워 목록 보기 (페이징)
     * (현재 유저를 팔로우 하고 있는 사람들(=follower_id)을 봐야하므로 follwing_id가 현재 유저여야 함)
     */
    Page<Follows> findByFollowing_UserId(Long id, Pageable pageable);
